...
suite('File Types Test Suite', () => {
  test('All file types should have required properties', () => {
    FILE_TYPES.forEach((fileType) => {
      ...
      assert.ok(
        fileType.extension.startsWith('.'),
        `File type ${fileType.id} extension should start with a dot`
      );
    });
  });
  ...
  test('Templates should be valid for their language', () => {
    ...
    templatedFileTypes.forEach((fileType) => {
      ...
      switch (fileType.id) {
        case 'java':
          assert.ok(
            fileType.template!.includes('public class'),
            'Java template should include class definition'
          );
          assert.ok(
            fileType.template!.includes('main'),
            'Java template should include main method'
          );
          break;
        case 'csharp':
        ...
        case 'c':
          assert.ok(
            fileType.template!.includes('#include'),
            'C/C++ template should include headers'
          );
          assert.ok(
            fileType.template!.includes('main'),
            'C/C++ template should include main function'
          );
          break;
        case 'go':
          assert.ok(
            fileType.template!.includes('package main'),
            'Go template should include package declaration'
          );
          assert.ok(
            fileType.template!.includes('func main'),
            'Go template should include main function'
          );
          break;
        case 'rust':
          assert.ok(
            fileType.template!.includes('fn main'),
            'Rust template should include main function'
          );
          break;
        case 'php':
          assert.ok(
            fileType.template!.includes('<?php'),
            'PHP template should include opening tag'
          );
          break;
        case 'html':
          assert.ok(
            fileType.template!.includes('<!DOCTYPE'),
            'HTML template should include DOCTYPE'
          );
          assert.ok(fileType.template!.includes('<html'), 'HTML template should include html tag');
          break;
        case 'xml':
          assert.ok(
            fileType.template!.includes('<?xml'),
            'XML template should include XML declaration'
          );
          break;
        case 'shell':
          assert.ok(
            fileType.template!.includes('#!/bin/bash'),
            'Shell template should include shebang'
          );
          break;
      }
    });
