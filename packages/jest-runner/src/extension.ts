...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register commands
    context.subscriptions.push(
        vscode.commands.registerCommand('jest-runner.runTest', async (testName: string, testFile: string) => {
            await runner.runTest(testName, testFile);
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('jest-runner.debugTest', async (testName: string, testFile: string) => {
    ...
    const testFileWatcher = vscode.workspace.createFileSystemWatcher('**/*.{test,spec}.{js,jsx,ts,tsx}');
