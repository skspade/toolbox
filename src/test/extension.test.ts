...
suite('Extension Test Suite', () => {
	...
	test('All commands should be registered', async () => {
		...
		assert.ok(commands.includes('scratches.createScratch'), 'createScratch command should be registered');
		assert.ok(commands.includes('scratches.listScratches'), 'listScratches command should be registered');
		assert.ok(commands.includes('scratches.deleteScratch'), 'deleteScratch command should be registered');
	...
	test('Create scratch command should work', async function() {
		...
		try {
			...
			assert.ok(activeEditor.document.fileName.includes('scratch-'), 'File should be a scratch file');
			assert.strictEqual(path.extname(activeEditor.document.fileName), '.js', 'File should have .js extension');
		} finally {
			stub.restore();
		}
	});
	...
	test('Keybindings should be defined', async () => {
		...
		assert.strictEqual(createScratchBinding.key, 'ctrl+alt+s', 'Windows/Linux key should be ctrl+alt+s');
		...
		assert.strictEqual(listScratchesBinding.key, 'ctrl+alt+o', 'Windows/Linux key should be ctrl+alt+o');
